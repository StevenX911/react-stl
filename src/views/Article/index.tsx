    ...
    useEffect(() => {
        getTopics().then(response => {
            ...
            const data: any = response.data.data.map((item: any, index: any) => {
                ...
                // console.log(author);
                return { key: index + 1, title, visit_count, reply_count, create_at, author: author.loginname };
            });
                ...
                if (item == 'visit_count') {
                    return {
                        ...
                        render: (text: any, record: any, index: any) => {
                            return <Tag color={record.visit_count > 1000 ? "red" : "green"}>{record.visit_count}</Tag>
                        }
                    }
                }
                if (item == 'reply_count') {
    ...
    return (
        <>
            <Card title="文章管理" extra={<Button type="dashed">导出文章</Button>}>
                <Table dataSource={DataSource} columns={Columns} pagination={{ total: Total, pageSize: 8 }} />
            </Card>
        </>
    )
}
